...
## Create results table (Supplemental Table 1)
...
## Create supplemental figure 2
## This code chunk still needs to be cleaned up so variable names etc. match the above
...
#################### SUPPLEMENTAL FIGURE 2 COL 1: Fric analysis: DOY ~ latitude 
sup1a<-list()
for(rowi in 1:nrow(datasets)) {
    #Filter Fric data
    speciesdata<-  filter(fric.data, name==datsets$name[rowi], name==datsets$name[rowi])
    ...
    mytitle<-paste(datasets$set[rowi])
    ...
    #Create plot
    sup1a[[i]] <- ggplot(speciesdata, aes(x=rndLat, y=SuccDay, color=as.factor(Group))) +
      geom_point(aes(color=as.factor(Group)), shape=3) +
      geom_point(onsetdata, aes(x=rndLat, y=SuccDay, color=as.factor(Group))) + 
      geom_point(termdata, aes(x=rndLat, y=SuccDay, color=as.factor(Group))) + 
      geom_segment(data=filter(speciesdata, onset==SuccDay | term==SuccDay), aes(x=rndLat, y=onset, xend=rndLat, yend=term)) + 
      geom_smooth(data=onsetdata, formula = str(fric.onset.lm$call),  method="lm", linetype=onsetline, color=onsetcolor, fill=onsetcolor) + 
      geom_smooth(data=termdata, formula = str(fric.term.lm$call), method="lm", linetype=termline, color=termcolor, fill=termcolor) + 
      scale_color_manual(values=c("black","red")) +
      xlim(xmin,max(speciesdata$rndLat)) + ylim(min(speciesdata$SuccDay),max(speciesdata$SuccDay)) +
      theme_light() +   theme(legend.position="none") + 
...
```
Still needed: 
Code to create columns 2 and 3 of supplemental figure 1.
Code to create supplemental table 2.
