...
date: "Current version 11/18/2020; initiated 3/9/2020"
...
If phest is not already installed, remove comment from install line
...
```
#### Data exploration: 
Data have now been formatted and stored into the "occur" tibble.
The Fric et al. (2020) analysis was done by region (North America vs. Europe) but data are not identified by region.
...
Unlike Fric et al., we included first day of the month records
Filters: 
...
new.data.summary<-occur1 %>%
  filter(between(alt,0,500), name!="Euphydryas aurinia") %>%
...
This chunk of code filters data for reanalysis and visualizes some differences
...
This chunk of code uses model outputs and compares them to the results of the Fric et al. analysis. It outputs Figure 2.
...
## Create results table (Supplemental Table 2)
...
summary(supptable2)
##This partial supplementary table 2 does not include the residual regression year results
#write.csv(supptable2,file="output/supp_table2_part.csv")

```

#### Create panels for Supplemental Figure 1
In this code chunk, we previously used lm.model$call references in geom_smooth, which created a string of outputs showing the calls. We have thus changed the geom_smooth parameters to simply reference  x, y, and method="lm"
...
```{r warning=F, fig.height = 7, fig.width = 10, fig.align = "center"}

#### COMBINE ALL PANELS INTO SUPPLEMENTAL FIGURE 1
