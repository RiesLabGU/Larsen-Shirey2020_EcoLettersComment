...
reanalysis.data<-reanalysis.data %>%
  filter(alt>=0, alt<=500, month %in% c(3:11)) %>%
  # calculate data availability by species, region, latitude & year
  group_by(name, region, rndLat, year) %>% 
  add_count(name="group_n") %>% ## n. observations per group
  filter(group_n>=10) %>%
...
#pheno$onset[pheno$onset<60]<-60
#pheno$offset[pheno$offset>330]<-330
...
This chunk of code uses estimated onset and offset phenometrics in mixed linear models to examine phenological patterns.

```{r}

#load("pheno2.RData")
#pheno2a$term[pheno2a$term>330]<-330
#pheno2a$onset[pheno2a$onset<60]<-60

datasets<-reanalysis.data %>%
  group_by(name, region) %>%
  tally()
...
for(rowi in 1:nrow(datasets)) {
  pheno.rowi<-reanalysis.data %>%
    ...
    group_by(name, region, year, rndLat) %>%
    #add_count(nday=SuccDay) %>%
    filter(group_n>=10) %>%
    summarize(onset=min(SuccDay), offset=max(SuccDay)) 
  onset.lm<-lm(onset~rndLat+year, data=pheno.rowi)
  offset.lm<-lm(offset~rndLat+year, data=pheno.rowi)
  onset.results[[rowi]]<-summary(onset.lm)$coefficients
  offset.results[[rowi]]<-summary(offset.lm)$coefficients
   rm(pheno.rowi, onset.lm, offset.lm)
}
...
fric.results.lat<-read_excel("fric_gbif/Fric_results.xlsx", sheet="models")
